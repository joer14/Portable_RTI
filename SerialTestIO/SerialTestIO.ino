...
void setup()
{
  ...
  pinMode(6, INPUT);   // digital sensor is on digital pin 2
...
void loop()
{
  ...
  if (Serial.available() > 0) {
    ...
    //thirdSensor = map(digitalRead(6), 0, 1, 0, 255);  
    thirdSensor = digitalRead(6);
    // send sensor values:
...
void establishContact() {
  while (Serial.available() <= 0) {
    Serial.println("9,0,0");   // send an initial string
    delay(500);
  }
}
